...
public class DragTween {
    ...
    public void update() {
        if (state == DragTweenState.Move) {
            ...
            curAngle = MathHelper.lerp(0.3f, curAngle, curAngle + gotoSpeed);
    ...
    private float calMaxAngle(float vel) {
        float maxAngle = (float) Math.abs(GUITweenClient.CONFIG.dragMaxAngle);
        return Math.clamp(vel * GUITweenClient.CONFIG.dragSensitivity, -maxAngle, maxAngle);
    }
}
