...
public class ContainerItemTween {
    ...
    public Tuple<Integer, Integer> getMoveTweenValue(float fromX, float fromY, float toX, float toY, Single single, int to, AbstractContainerMenu menu) {
        ...
        float moveDuration = GUITweenConfig.windowItem.moveDuration.get().floatValue();
        Ease moveEase = GUITweenConfig.windowItem.moveEase.get();
    ...
    public void addFinishTween(Slot slot) {
        if (!GUITweenConfig.isEnableFinishItem())
            return;

        if (!finishTween.containsKey(slot)) {
            finishTween.put(slot, 0f);

            NeoForge.EVENT_BUS.post(new PlayGuiSoundEvent(PlayGuiSoundEvent.SoundType.MOVE_FINISH));
        }
    }
    ...
    public Float getFinishScale(Slot slot) {
        ...
        float punchStrength = GUITweenConfig.windowItem.finishPunchStrength.get().floatValue();
        float finishDuration = GUITweenConfig.windowItem.finishDuration.get().floatValue();
    ...
    public void addPickupTween(Slot slot) {
        if (!GUITweenConfig.isEnablePickupItem())
            return;

        pickupTweenMap.put(slot, 0f);

        NeoForge.EVENT_BUS.post(new PlayGuiSoundEvent(PlayGuiSoundEvent.SoundType.PICK_UP));
    }
    ...
    public Float getPickupScale(Slot slot) {
        ...
        float pickupDuration = GUITweenConfig.windowItem.pickupDuration.get().floatValue();
        Ease pickupEase = GUITweenConfig.windowItem.pickupEase.get();

        float scale = TweenUtil.tween(GUITweenConfig.windowItem.clickItemScale.get().floatValue(), 1, tick / pickupDuration, pickupEase);
    ...
    public Float getQuickCraftScale(Slot slot) {
        ...
        float quickCraftDuration = GUITweenConfig.windowItem.quickCraftDuration.get().floatValue();
